...
while repeat == "y":
    ...
    if choiceCmd == "1":
        ...
        copycol=ces2gTable["CELL"]
        ces2gTable.insert(6, "ifBSnameIO", copycol)
        ces2gTable["ifBSnameIO"] = ces2gTable["ifBSnameIO"].str[:6]
        ...
        copycol=ces4gTable["Имя системного модуля"]
        ces4gTable.insert(6, "ifBSnameIO", copycol)
        ces4gTable["ifBSnameIO"] = ces4gTable["ifBSnameIO"]
        #print("Таблицы (ces2gTable, ces4gTable) 2g, 4g из сайта CES для того, чтобы посмотреть какие данные на сайте не заполнены:")
        #print(ces2gTable)
        #print(ces4gTable)
        ...
        #14 Добработать программу так, чтобы она считывала данные IO.
        allBs2gTable=allBs2gTable.drop("ifRegIO", axis=1)
        allBs2gTable=allBs2gTable.drop("ifBSnameIO", axis=1)
        allBs4gTable=allBs4gTable.drop("ifRegIO", axis=1)
        allBs4gTable=allBs4gTable.drop("ifBSnameIO", axis=1)
        ...
        allBs4gTable = allBs4gTable.drop_duplicates()
        print("Таблицы (allBs2gTable, allBs4gTable) с довесами и новыми базовыми станциями 2g, 4g:")
        print(allBs2gTable)
        print(allBs4gTable)
        with open("output.log", "a") as outfile:
        ...
        allBs2gTable.to_csv('tempfile2g.csv', sep=';', index=False, header=False, encoding='UTF-8-SIG')
        allBs4gTable.to_csv('tempfile4g.csv', sep=';', index=False, header=False, encoding='UTF-8-SIG')
        with open("output.log", "a") as outfile:
